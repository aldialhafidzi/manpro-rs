<div class="content-wrapper">
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-7">
          <h1 class="m-0 text-dark">Ruang Rawat Kelas 2</h1>
        </div>

        <div class="btn-group" style="margin: 0px 5px 0px 5px">
          <button type="button" class="btn btn-default" style="width: 140px">Kelas 2</button>
          <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
            <span class="caret"></span>
          </button>
          <ul class="dropdown-menu" role="menu">
            <li><a class="dropdown-item " href="<?= base_url() ?>ruangrawat/ruang_vvip">VVIP</a></li>
            <li><a class="dropdown-item " href="<?= base_url() ?>ruangrawat/ruang_vip">VIP</a></li>
            <li class="divider"></li>
            <li><a class="dropdown-item " href="<?= base_url() ?>ruangrawat/ruang_k1">Kelas 1</a></li>
            <li><a class="dropdown-item active" href="<?= base_url() ?>ruangrawat/ruang_k2">Kelas 2</a></li>
            <li><a class="dropdown-item " href="<?= base_url() ?>ruangrawat/ruang_k3">Kelas 3</a></li>
          </ul>
        </div>
      
        <form action="#" method="get" class="sidebar-form" style="margin: 0px 5px 0px 5px">
          <div class="input-group align-right">
          <input type="text" name="q" class="form-control" placeholder="Search...">
            <span class="input-group-btn">
              <button type="submit" name="search" id="search-btn" class="btn btn-flat"><i class="fa fa-search"></i>
              </button>
            </span>
          </div>
        </form>
      
      </div>
    </div>
  </div>

  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-6 col-xs-6">
          <div class="small-box bg-white">
            <div class="inner">
              <h4>Ruang Melati</h4>
              <table class="table table-bordered text-center">
                <tbody><tr>
                  <td>
                    <a href="ruang_rawat_info" type="button" class="btn btn-block btn-default" onclick="a">Kasur 1</a>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 2</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 3</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 4</button>
                  </td>
                </th>
                </tr>
                <tr>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 5</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 6</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 7</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 8</button>
                  </td>
                </th>
                </tr>
              </tbody></table>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-xs-6">
          <div class="small-box bg-white">
            <div class="inner">
              <h4>Ruang Melati</h4>
              <table class="table table-bordered text-center">
                <tbody><tr>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 1</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 2</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 3</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 4</button>
                  </td>
                </th>
                </tr>
                <tr>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 5</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 6</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 7</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 8</button>
                  </td>
                </th>
                </tr>
              </tbody></table>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-xs-6">
          <div class="small-box bg-white">
            <div class="inner">
              <h4>Ruang Melati</h4>
              <table class="table table-bordered text-center">
                <tbody><tr>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 1</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 2</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 3</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 4</button>
                  </td>
                </th>
                </tr>
                <tr>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 5</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 6</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 7</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 8</button>
                  </td>
                </th>
                </tr>
              </tbody></table>
            </div>
          </div>
        </div>
    
        <div class="col-md-6 col-xs-6">
          <div class="small-box bg-white">
            <div class="inner">
              <h4>Ruang Melati</h4>
              <table class="table table-bordered text-center">
                <tbody><tr>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 1</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 2</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 3</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 4</button>
                  </td>
                </th>
                </tr>
                <tr>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 5</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 6</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 7</button>
                  </td>
                  <td>
                    <button type="button" class="btn btn-block btn-default">Kasur 8</button>
                  </td>
                </th>
                </tr>
              </tbody></table>
            </div>
          </div>
        </div>
    
      </div>  
    </div>
  </div>
</div>
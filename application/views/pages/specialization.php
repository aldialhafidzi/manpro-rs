<div class="content-wrapper">
    ...
    <div class="modal fade" id="modal_add" tabindex="-1" role="dialog" aria-labelledby="modal_addTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form action="<?= base_url() ?>specialization/create" method="POST">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Buat Specialization</h5>
    ...
    <div class="modal fade" id="modal_hapus" tabindex="-1" role="dialog" aria-labelledby="modal_hapusTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Hapus Specialization</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body" style="padding: 1rem;">
                    <div class="row mb-3">
                        <div class="col-12">
                            <p>Apa anda yakin akan menghapus data specialization ini ? </p>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <form action="<?= base_url() ?>specialization/delete" method="POST">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <button class="btn btn-primary btn-sm" onclick="initBuatSpecialization()"> <i class="fas fa-plus"></i> &nbsp;Buat Specialization Baru</button>
                                        ...
                                        echo '
                                            <tr>
                                                ...
                                                <td style="width:160px;text-align:center;">
                                                    <button value="' . $spec->id . '" class="btn-hapusSpecialization btn btn-sm btn-danger" onclick="deleteSpecialization(' . $spec->id . ')">
                                                        <i class="far fa-trash-alt"></i> &nbsp; Hapus
                                                    </button>
                                                    <button value="' . $spec->id . '" class="btn-lihatSpecialization btn btn-sm btn-info" onclick="showDetailSpecialization(' . $spec->id . ')">
...
<script>
    ...
    function initBuatSpecialization() {
        $('#exampleModalLongTitle').html('Buat Specialization');
        $('#kode').val('');
        $('#nama').val('');
    ...
    function showDetailSpecialization(id) {
        $.ajax({
            url: `<?= base_url() ?>specialization/get?specialization_id=${id}`,
            type: 'GET',
            dataType: 'JSON',
            success: function(data) {
                $('#exampleModalLongTitle').html('Update Specialization');
                $('#kode').val(data.kode);
                $('#nama').val(data.nama);
                $('#specialization_id').val(data.id);
                $('#modal_add').modal('show');
            },
            error: function(err) {}
        });
